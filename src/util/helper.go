...
const (
	...
	namePattern = `^[a-z]([-a-z0-9])?([a-z0-9]([-a-z0-9]*[a-z0-9])?)*$`
	// 带点的名称的正则匹配, 合法的字符有 0-9, A-Z, a-z,-,_
	namePointPattern = `^[a-z]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*$`
)
...
func StringInArray(arr []string, dist string) bool {
	for _, v := range arr {
		if v == dist {
			return true
		}
...
// RemoveDuplicateInt64 去重
func RemoveDuplicateInt64(array []int64) []int64 {
	var result []int64
...
func Errors(errs, err error) error {
	if errs != nil {
		return errors.Wrap(errs, err.Error())
	}
	return err
}
